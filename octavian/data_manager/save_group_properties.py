...
def save_group_properties(data_manager: DataManager, filename: str) -> None:
  ...
  with h5py.File(filename, 'w') as f:
    ...
    for dataset_name, column in config['dataset_columns'].items():
      if dataset_name in ['glist', 'slist', 'dmlist', 'bhlist']:
        dt = h5py.vlen_dtype(np.dtype('int32'))
      else:
        dt = None

      if np.all(np.isin(column, halo_columns)):
        halo_data.create_dataset(dataset_name, data=data_manager.group_data['halos'][column].to_numpy(), dtype=dt, compression=1)
      if np.all(np.isin(column, galaxy_columns)):
        galaxy_data.create_dataset(dataset_name, data=data_manager.group_data['galaxies'][column].to_numpy(), dtype=dt, compression=1)
