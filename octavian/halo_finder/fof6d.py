...
def run_fof6d(data_manager: DataManager, nproc: int = 1) -> None:
  ...
  # this is from the previous code; I think it initialises galaxy IDs (so could maybe move to top of mpi section? (done now))
  for ptype in config['ptypes']:
  ...
  if data_manager.comm:
    ...
    halo_ids_local = halo_ids # serial case

  halo_group_local = fof_halos[fof_halos['HaloID'].isin(halo_ids_local)].groupby('HaloID', observed=True) # uses mask to select local halos then groups them by ID

  # we now find the galaxies in each worker's halos
  if data_manager.comm: # use fof in the worker's own group
    galaxies_local = [run_fof6d_in_halo(halo, kernel_table,
                                         config['MINIMUM_STARS_PER_GALAXY'], 
                                         fof_LL, vel_LL)
                      for idx, halo in halo_group_local] # find the galaxies in the halo
  else: # serial case
    galaxies_local = Parallel(n_jobs=nproc)(delayed(run_fof6d_in_halo)(halo, kernel_table, 
                                                                       config['MINIMUM_STARS_PER_GALAXY'], 
                                                                       fof_LL, vel_LL) 
                                            for idx, halo in halo_group_local)
    
  galaxies_local = [galaxy for galaxy_list in galaxies_local for galaxy in galaxy_list if len(galaxy_list) != 0] # flatten list (potential memory problem)

  # now tot up the number of galaxies and send this to rank 0
  num_galaxies_local = len(galaxies_local)
  ...
  for i, galaxy in enumerate(galaxies_local): 
    gal_id = galaxy_id_offsets[data_manager.rank] + i
    for ptype, ptype_indexes in galaxy:
        data_manager.data[ptype].loc[ptype_indexes, 'GalID'] = gal_id

  # this then makes the galaxy IDs a category (unsure where this leads thus far, memory efficiency?)
  for ptype in config['ptypes']:
